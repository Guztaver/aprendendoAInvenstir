...
<template>
  ...
  <div id="text_contet house">
    ...
    <p>Você teria R${{ formatPrice(immobileRentInAMont * 12 * immobileInvestmentInYears) }} retornados de aluguel, ao longo de {{ immobileInvestmentInYears }} anos, e se vendesse, teria R${{ formatPrice(imobileSellPrice - immobilePrice - immobileBuyAndSellCost) }} de retorno.</p>
  </div>
  ...
  <div id="text_contet actions">
    ...
    <p> 
      Você teria R${{ formatPrice((sharesInvestedPrice * Math.pow(1 + (sharesRevenue / 100), sharesInYears)).toFixed(2))}} em ações, se tivesse investido {{ sharesInvestedPrice }} em {{ sharesInYears }} com a taxa anual de {{ sharesRevenue }}%.
    </p>
  </div>
  ...
  <div>
    ...
    <div>
      ...
      <p> Você teria R${{ formatPrice((savingsInitialInvestedPrice * Math.pow(1 + (savingsTaxesFees / 100), savingsInYears)).toFixed(2))}} na poupança, se tivesse guardado ao longo de {{ savingsInYears }}, com a taxa anual de juros a {{ savingsTaxesFees }}%. </p>
    </div>
  </div>
</template>
